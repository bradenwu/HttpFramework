...
public class HttpRequestGeneralParams {
	...
	public static String HttpRequestGeneralParamsToString(HttpRequestGeneralParams mParams){
		...
		if( (mParams.mMaskFlag&BODYMASK) != 0){
			Log.d("xiaohuan","enter body part "+Integer.toHexString(mParams.mMaskFlag&BODYPART));
			if((mParams.mMaskFlag&BODYPART_WITHENTITY) != 0){
				
				Log.d("xiaohuan","enter body entity part:"+Integer.toHexString(mParams.mMaskFlag&BODYPART_WITHENTITY));
			...
			if( (mMaskFlag&BODYMASK) != 0){
				if((mMaskFlag&BODYPART_WITHENTITY) != 0){
					if(TextUtils.isEmpty(resultJson.optString("body_string_enty_encoding")))
						mResult.addBodyEntity(resultJson.optString("body_string_enty"));//now add body entity
					else 
						mResult.addBodyEntity(resultJson.optString("body_string_enty"), resultJson.optString("body_string_enty_encoding"));
				}

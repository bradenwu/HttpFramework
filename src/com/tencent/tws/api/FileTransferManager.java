...
public class FileTransferManager
{
     ...
     *            ，源文件路径
     * @param dstPath
     *            ，目标设备文件目录，这里默认是null，代表默认放到/sdcard/twsReceived目录下
     * @返回类型：int
     */
    ...
    private class FileTransferTask implements Task
    {
        ...
        private IFileTransferListener createIFileTransferListener(final FileTransferListener listener)
        {
            IFileTransferListener iListener = new IFileTransferListener.Stub()
            {
                @Override
                public void onTransferProgress(final long requestId, final String fileName, final long progress) throws RemoteException
                {
                ...
                @Override
                public void onTransferError(final long requestId, final String fileName, final int errorCode) throws RemoteException
                {
                    mUiHandler.post(new Runnable()
                    {
                        @Override
                ...
                @Override
                public void onTransferComplete(final long requestId, final String filePath) throws RemoteException
                {
                    mUiHandler.post(new Runnable()
                    {
                        @Override
                ...
                @Override
                public void onTransferCancel(final long requestId, final int reason) throws RemoteException
                {
                    mUiHandler.post(new Runnable()
                    {
                        @Override
    ...
    /**
     * Helper class to manage a queue of pending tasks to send to lbs service
     * channel listeners.
     */
...
//        private Set<String>                                    mCachedEnabledPackages   = new HashSet<String>();
        ...
        /**
         * Queue a new task to be sent to all listeners. This function can be
        ...
        public void onServiceConnected(ComponentName componentName, IBinder iBinder)
        {
            ...
            mHandler.obtainMessage(MSG_SERVICE_CONNECTED, new ServiceConnectedEvent(componentName, iBinder)).sendToTarget();
        }

        /**
         * Check the current list of enabled listener packages and update the
         * records map accordingly.
         */
...
//            mCachedEnabledPackages = enabledPackages;
            ...
            //step1: 找出 PackageManager里所有intent的filter为ACTION_BIND_SIDE_CHANNEL的service,添加到mRecordMap,为下一步binder这些对应的service做准备
            List<ResolveInfo> resolveInfos = mContext.getPackageManager().queryIntentServices(new Intent().setAction(FILE_SERVICE_ACTION), PackageManager.GET_SERVICES);
...
//                }
            ...
            Iterator<Map.Entry<ComponentName, ListenerServieRecord>> it = mRecordMap.entrySet().iterator();
            while (it.hasNext())
            {
        ...
        /**
         * Ensure we are already attempting to bind to a service, or start a new
         * binding if not.
         * 
        ...
        /**
         * Perform a processing step for a listener. First check the bind state,
         * then attempt to flush the task queue, and if an error is encountered,
         * schedule a retry.
         */
        ...
        private static class ListenerServieRecord
        {
            ...
            /**
             * Number of retries attempted while connecting to this listener
             * service
             */
    ...
    {
        long dmApiVersion = ApiVersionManager.getApiVersion(TwsApiName.LBS_API, mContext);
        long localApiVersion = TwsApiVersion.LBS_API_VERSION;
        return dmApiVersion >= localApiVersion ? true : false;
    }
